...
export function buildPricingSummary(
  ...
  const dimensionBaseScores = DIMENSION_META.map((dimension) => {
    ...
    const score = clamp(Math.round(answeredQuestions * 8 + averageWords / 6 + keywordHits * 7), 12, 100)
  ...
  const exposurePercent = clamp(0.45 + documentComplexityScore / 12 + matterProfileScore / 55, 0.6, 9.5)
